(() => {
  const config = window.FLORALIS_CONFIG || { mode: 'demo' };
  const sessionKey = 'floralis.session.v1';
  let session = null;
  let businessId = null;
  ...
  async function loadWorkspace() {
    ...
    return {
      settings: { businessName: business.name || 'FLORALES', legalName: business.legal_name || '', nit: business.tax_id || '', email: business.email || '', phone: business.phone || '', address: business.address || '', postalCode: business.postal_code || '', city: business.city || '', currency: business.currency || 'Bs', baseCurrency: 'BOB', exchangeRates: { BOB: 1, BRL: .5, USD: .14 }, language: settings?.[0]?.theme?.language || 'es', accent: settings?.[0]?.theme?.accent || '#652276', appearance: settings?.[0]?.theme || {}, posLayout: settings?.[0]?.pos_layout || {}, ...(settings?.[0]?.app_config || {}) },
  ...
  async function saveBusiness(settings) {
    ...
    const rows = await request(`/rest/v1/businesses?id=eq.${businessId}`, { method: 'PATCH', body: { name: settings.businessName, legal_name: settings.legalName, tax_id: settings.nit, email: settings.email || null, phone: settings.phone, address: settings.address || null, postal_code: settings.postalCode || null, city: settings.city, currency: settings.currency || 'Bs' } });
    return rows?.[0] || settings;
  }
  async function saveSettings(settings) { const { appearance, posLayout, language, accent } = settings, appConfig = { options: settings.options || {}, wholesale: Boolean(settings.wholesale), cashClosing: settings.cashClosing || {}, documentTemplates: settings.documentTemplates || {}, watermarkOpacity: settings.watermarkOpacity, logo: settings.logo || '', email: settings.email || '', address: settings.address || '', postalCode: settings.postalCode || '', website: settings.website || '', zone: settings.zone || '', pointOfSale: settings.pointOfSale || '', authorizationCode: settings.authorizationCode || '', baseCurrency: settings.baseCurrency || 'BOB', exchangeRates: settings.exchangeRates || { BOB: 1, BRL: .5, USD: .14 }, saleReceiptTemplate: settings.saleReceiptTemplate === 'modern' ? 'modern' : 'classic-form' }; return upsert('business_settings', { theme: { ...(appearance || { accent }), language: language || 'es' }, pos_layout: posLayout || {}, app_config: appConfig }, 'business_id'); }
  ...
  async function registerSale(payload) { if (!isRemote()) return null; await request('/rest/v1/rpc/register_sale_v2', { method: 'POST', headers: { 'X-Idempotency-Key': payload.clientSaleId || '' }, body: { p_business_id: businessId, p_client_id: payload.clientId || null, p_payment_method: payload.payment, p_items: payload.items, p_currency_code: payload.currency || 'BOB', p_exchange_rate: Number(payload.exchangeRate || 1), p_display_total: Number(payload.displayTotal || 0), p_notes: payload.notes || null, p_client_sale_id: payload.clientSaleId, p_kind: payload.kind || 'Venta' } }); const rows = await request(`${scoped('sales')}&client_sale_id=eq.${encodeURIComponent(payload.clientSaleId)}&select=id,sale_number&limit=1`); return rows?.[0] ? { saleId: rows[0].id, saleNumber: rows[0].sale_number } : null; }
  ...
  async function ensureTrainingProduct() { if (!isRemote() || session?.role !== 'admin') return null; return upsert('products', { code: 'TEST-000000', ean: null, name: 'Producto test', category: 'Tutorial', stock: 100, min_stock: 0, cost: 0, price: 100, wholesale_price: 100, unit: 'Unidad', notes: 'Producto para practicar el flujo del PDV sin afectar los resultados reales.', image_url: './assets/icon-512.png', active: true }, 'business_id,code'); }
  async function addCashMovement(movement) { return upsert('cash_movements', { kind: movement.type, description: movement.description, amount: movement.amount, employee_name: session?.name || '', currency_code: movement.currency || 'BOB', exchange_rate: Number(movement.exchangeRate || 1), display_amount: Number(movement.displayAmount ?? movement.amount), amount_bob: Number(movement.amountBob || 0), amount_brl: Number(movement.amountBrl || 0) }); }
